...
import { BorrarTaller, CargarTaller, CargaTallerById } from '../interfaces/cargar-talleres.interface';
...
export class TalleresService {
    ...
    cargarTaller(desde: number = 0) {
        const url = `${taller_url}/taller?desde=${desde}`;
        return this.http.get<CargarTaller>(url, this.headers)
            .pipe(
                map(resp => {
                    const talleres = resp.talleres.map(
