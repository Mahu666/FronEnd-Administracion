...
import { BorrarSitio, CargarSitio, CargaSitioById } from '../interfaces/cargar-sitios.interface';
...
export class SitiosService {
                ...
                map(resp => {
                    const sitios = resp.sitios.map(
                        sites => new Sitio(sites.nombreSitio, sites.imgPrincipal, sites.video, sites.descripcion, sites.banner, sites.nombreContacto1, sites.telContacto1,
                            ...
                            sites.horarioClose, sites.ubicacion, sites.licencia, sites.facebook, sites.instagram, sites.sitioWeb, sites.categoria, sites.usuario,
                            sites.municipio, sites.status, sites._id));
                    return {
                        total: resp.total,
                        sitios
                    };
    ...
    actualizarSitio(nombreSitio: string, imgPrincipal: string[], video: string, descripcion: string, banner: string, nombreContacto1: string, telContacto1: string,
        ...
        ubicacion: string, licencia: Date, facebook: string, instagram: string, sitioWeb: string, categoria: Categoria, usuario: Usuario, municipio: Municipio, status: boolean, _id?: string,) {
        console.log(_id)
        const url = `${sit_url}/sitio/${_id}`;
        const data = {
            nombreSitio,
    ...
    crearSitio(nombreSitio: string, imgPrincipal: string[], video: string, descripcion: string, banner: string, nombreContacto1: string, telContacto1: string, correoContacto1: string,
        ...
        ubicacion: string, licencia: Date, facebook: string, instagram: string, sitioWeb: string, categoria: Categoria, usuario: Usuario, municipio: Municipio, status: boolean) {
