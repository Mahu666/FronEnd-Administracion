...
import { BorrarEvento, CargarEvento, CargaEventoById } from '../interface/cargar-eventos.interface';
...
export class EventosService {
    ...
    cargarEventoBySitio(_id: any) {
        const url = `${eve_url}/evento/sitio/${_id}`;
        return this.http.get<CargarEvento>(url, this.headers)
            .pipe(
                map(resp => {
                    const eventos = resp.resultados.map(
