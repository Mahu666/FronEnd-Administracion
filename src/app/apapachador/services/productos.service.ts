...
import { BorrarProducto, CargarProducto, CargaProductoById } from '../interface/cargar-productos.interface';
...
export class ProductosService {
    ...
    cargarProductoBySitio(_id: any) {
        const url = `${pro_url}/producto/sitio/${_id}`;
        return this.http.get<CargarProducto>(url, this.headers)
            .pipe(
                map(resp => {
                    const productos = resp.resultados.map(
